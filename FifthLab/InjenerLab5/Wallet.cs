...
namespace InjenerLab5
{
    public class Wallet
    {
        ...
        public void addMoney(String Currency, int Value)
        {
            for(int i = 0; i < walletAccount.Count(); i++) 
            {
                ...
                if (walletAccount[i].Currency == Currency) 
                {
                    ...
                    Console.WriteLine("Пополенен счет: " + Currency + " на сумму: " + Value.ToString());
                ...
                else if (count > 0)
                {
                    ...
                    Console.WriteLine(Currency + ": 0, такого счёта нет, заберите деньги!");
                    Console.ForegroundColor = ConsoleColor.White;
                }
                else
                {
        ...
        public void removeMoney(String Currency, int Value)
        {
            ...
            for (int i = 0; i < walletAccount.Count(); i++)
            {
                if (walletAccount[i].Currency == Currency)
                {
                    ...
                    money.addHistory("С вашего счёта " + Currency + " списано: ", Value);
                    Console.ForegroundColor = ConsoleColor.White;
                }
                else if (count > 0) 
                {
        ...
        public string toString()
        {
            ...
            string writter = "Cодержимое кошелька: { ";
            Console.Write("Cодержимое кошелька: { ");
            for (int i = 0; i < walletAccount.Count(); i++)
            {
